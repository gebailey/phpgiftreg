...
if (!empty($_REQUEST["action"])) {
	...
	if ($action == "delete") {
		try {
			...
			while ($row = $stmt->fetch()) {
				...
				if ($buyerid != null) {
					sendMessage($userid,
						$buyerid,
						$row["description"] . " that you " . (($bought == 1) ? "bought" : "reserved") . " $quantity of for " . $row["fullname"] . " has been deleted.  Check your reservation/purchase to ensure it's still needed.",
						$smarty->dbh(),
						$smarty->opt());
				}
			}
		...
		if (!$haserror) {
			...
			$stmt = $smarty->dbh()->prepare("INSERT INTO {$opt["table_prefix"]}items(userid,submitterid,description,price,source,category,url,ranking,comment,quantity" . ($image_base_filename != "" ? ",image_filename" : "") . ") " .
